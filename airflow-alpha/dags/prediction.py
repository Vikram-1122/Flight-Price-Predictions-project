...
def is_server_available(url, timeout=5):
    """Check if the server is reachable within the specified timeout."""
    try:
...
def prediction_dag():
    ...
    def make_predictions(files: list):
        ...
        for file in files:
            ...
            if not is_server_available(PREDICTION_API_URL):
                raise ConnectionError("Prediction server is unavailable.")
